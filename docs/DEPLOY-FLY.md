...
- Fly inyecta `PORT=8080` automáticamente; debe coincidir con `internal_port`.
- La app escucha en **`0.0.0.0:${PORT}`** (no `127.0.0.1`).
- No fije `PORT=3000` en `.env`, secrets ni `[env]` de `fly.toml`.
...
4. Verifique logs (no debe aparecer `injecting env from .env`; puerto **8080**; conexión PostgreSQL OK):
