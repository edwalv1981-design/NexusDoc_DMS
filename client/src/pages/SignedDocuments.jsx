...
const SignedDocuments = () => {
  ...
  const fetchDocuments = async () => {
    try {
      ...
      toast.error(t('signedDocs.errorLoad'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      if (editingDocId) {
        ...
        toast.success(`${t('userDocs.docUploaded')}: ${res.data.status}`);
      }
      setFile(null);
      ...
      toast.error(err.response?.data?.msg || t('toast.saveError'));
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleDelete = async (id, filename) => {
    if (!window.confirm(t('signedDocs.confirmDelete', { filename }))) return;
    try {
      const token = localStorage.getItem('token');
  ...
  return (
    <div style={{ padding: '30px', maxWidth: '1000px', margin: '0 auto' }}>
      ...
      <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: RADIUS, padding: '15px', marginBottom: '30px', display: 'flex', gap: '15px', alignItems: 'flex-start' }}>
        ...
        <div>
          <h4 style={{ color: '#166534', margin: '0 0 5px 0', fontSize: '14px' }}>{t('signedDocs.validationTitle')}</h4>
          <p style={{ color: '#15803d', margin: 0, fontSize: '13px', lineHeight: 1.5 }}>
            {t('signedDocs.validationBody')}
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: '30px', alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: 1, background: 'white', padding: '25px', borderRadius: RADIUS, border: `1px dashed #cbd5e1` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
            ...
            <h3 style={{ fontSize: '16px', fontWeight: 700, margin: 0 }}>
              {editingDocId ? t('signedDocs.replacePdf') : t('signedDocs.uploadNewPdf')}
            </h3>
          </div>
          
          <form onSubmit={handleUpload} style={{ display: 'flex', flexDirection: 'column', gap: 15 }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <label style={{ fontSize: '11px', fontWeight: 700, color: '#64748b' }}>{t('signedDocs.pdfLabel')}</label>
              <input 
                id="sig-upload-input"
                type="file" 
                accept=".pdf" 
        ...
        {/* Panel de Documentos */}
        <div style={{ flex: 1.5 }}>
          <h3 style={{ fontSize: '16px', fontWeight: 700, marginBottom: '20px' }}>{t('signedDocs.deliveredDocs')}</h3>
          ...
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: '40px 20px', background: 'white', borderRadius: RADIUS, border: `1px solid ${BORDER}` }}>
                ...
                <p style={{ color: '#94a3b8', margin: 0, fontSize: '14px', fontWeight: 600 }}>{t('signedDocs.noDocs')}</p>
            </div>
          ) : (
                        ...
                        <td style={{ padding: '12px 15px' }}>
                            {doc.signatureStatus === 'Firma Detectada' ? (
                                <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', background: '#dcfce7', color: '#16a34a', padding: '4px 8px', borderRadius: '4px', fontSize: '10px', fontWeight: 700 }}>
                                    <CheckCircle size={12} /> {t('signedDocs.signatureDetected')}
                                </span>
                            ) : (
                                <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', background: '#fef3c7', color: '#d97706', padding: '4px 8px', borderRadius: '4px', fontSize: '10px', fontWeight: 700 }}>
                                    <Clock size={12} /> {t('signedDocs.signaturePending')}
                                </span>
                            )}
                        </td>
