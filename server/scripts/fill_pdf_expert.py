...
def insert_text_scaled(page, rect, text, fontname="Helvetica", max_fontsize=9, min_fontsize=6, color=(0,0,0)):
    ...
    # Ajuste fino de baseline para que no choque con la línea inferior
...
def fill_pdf_universal_engine(data, output_path, template_name, master_config, custom_template_path=None):
    ...
    # ══════════════════════════════════════════════════════════════════════════════
    # ██████  MOTOR CORPORACIÓN (RÉPLICA 100% - SIN ANEXOS BASURA) █████████████████
    # ══════════════════════════════════════════════════════════════════════════════
    if template_name == "corporacion" or "corpNameSA" in data:
        ...
        # 3. DIRECTORES (GRID DINÁMICO)
        # Offset vertical exacto del formato PTL: 19.5 puntos
        ROW_H = 19.45
        ...
        def fill_director_block(page, d_idx, x_start, y_base, is_split=False):
            ...
            if not is_split:
                ...
                # Layout Split (Director 3 o huérfanos)
                for idx, f in enumerate(fields[:9]): # Datos personales (Izq)
                    val = d.get(f)
                    if val:
                        rect = fitz.Rect(185, y_base + (idx * ROW_H) - 12, 300, y_base + (idx * ROW_H) + 2)
                        insert_text_scaled(page, rect, str(val))
                for idx, f in enumerate(fields[9:]): # Ubicación (Der)
                    val = d.get(f)
                    if val:
                        # Address va en la primera fila de la derecha, City y Country siguen
        ...
        fill_director_block(page1, 0, 185, y_dir_start) # D1
        fill_director_block(page1, 1, 465, y_dir_start) # D2
        
        y_dir3 = find_anchor_y(page1, "Director 3") or 775
        fill_director_block(page1, 2, 0, y_dir3, is_split=True) # D3 (Split)

        # 4. MANEJO DE DIRECTORES EXTRA (CLONACIÓN DE PÁGINA)
        if len(directors) > 3:
            src_doc = fitz.open(pdf_path)
            for i in range(3, len(directors), 2):
                doc.insert_pdf(src_doc, from_page=0, to_page=0, start_at=len(doc)-1)
                new_p = doc[len(doc)-2]
                # Limpiamos áreas superiores para reutilizar el grid de directores
                ...
                # Rellenamos el par en la nueva página usando el mismo grid
                fill_director_block(new_p, i, 185, y_dir_start)
                if i+1 < len(directors):
                    fill_director_block(new_p, i+1, 465, y_dir_start)
            if 'src_doc' in locals(): src_doc.close()

        # 5. PÁGINA FINAL (OFFICERS & SHAREHOLDERS)
        pageF = doc[len(doc)-1]
        ...
        if data.get("declarationName"):
            pageF.insert_text((150, y_decl), str(data["declarationName"]), fontsize=11, fontname="Helvetica-Bold")
        y_date = find_anchor_y(pageF, "Date // Fecha") or 885
        if data.get("declarationDate"):
            pageF.insert_text((220, y_date), str(data["declarationDate"]), fontsize=11)

    # ══════════════════════════════════════════════════════════════════════════════
    # ██████  MOTOR FONDOS (SFAR) - ZONA PROTEGIDA █████████████████████████████████
    # ══════════════════════════════════════════════════════════════════════════════
    else:
        page1 = doc[0]
        ...
        # Dirección de Custodia (Autocalibración Dinámica)
