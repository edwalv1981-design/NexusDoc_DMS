...
const generateIdCode = `const generateId = () => Date.now().toString(36) + Math.random().toString(36).substring(2);\n\n`;

content = content.replace(
  "const CorporacionForm = ({ initialData, onSave, saving }) => {",
  generateIdCode + "const CorporacionForm = ({ initialData, onSave, saving }) => {"
);
...
content = content.replace(
  /<div className=\"corp-field\"><label>\{lang === 'en' \? 'Date of birth' : 'Fecha de nacimiento'\}<\/label><input type=\"date\" className=\"corp-input\" style=\{getArrayErrorStyle\('dignitaries', i, 'birthDate'\)\} value=\{dig.birthDate\} onChange=\{e => updateDignitary\(i, 'birthDate', e.target.value\)\} onBlur=\{[^>]+\} \/><ArrayFieldError array=\"dignitaries\" index=\{i\} field=\"birthDate\" \/><\/div>/g,
  '{dig.entityType !== "company" && <div className="corp-field"><label>{lang === \'en\' ? \'Date of birth\' : \'Fecha de nacimiento\'}</label><input type="date" className="corp-input" style={getArrayErrorStyle(\'dignitaries\', i, \'birthDate\')} value={dig.birthDate} onChange={e => updateDignitary(i, \'birthDate\', e.target.value)} onBlur={() => handleArrayFieldBlur(\'dignitaries\', i, \'birthDate\')} /><ArrayFieldError array="dignitaries" index={i} field="birthDate" /></div>}\n                        {dig.entityType === "company" && <div className="corp-field"><label>{lang === \'en\' ? \'Date of Incorporation\' : \'Fecha de constitución\'}</label><input type="date" className="corp-input" style={getArrayErrorStyle(\'dignitaries\', i, \'birthDate\')} value={dig.birthDate} onChange={e => updateDignitary(i, \'birthDate\', e.target.value)} onBlur={() => handleArrayFieldBlur(\'dignitaries\', i, \'birthDate\')} /><ArrayFieldError array="dignitaries" index={i} field="birthDate" /></div>}'
);

content = content.replace(
  /<div className=\"corp-field\"><label>\{lang === 'en' \? 'Registration Number' : 'Número de Registro'\}<\/label><input className=\"corp-input\" style=\{getArrayErrorStyle\('dignitaries', i, 'registrationNumber'\)\} value=\{dig.registrationNumber \|\| ''\}/g,
  '{dig.entityType !== "company" && <div className="corp-field"><label>{lang === \'en\' ? \'Registration Number\' : \'Número de Registro\'}</label><input className="corp-input" style={getArrayErrorStyle(\'dignitaries\', i, \'registrationNumber\')} value={dig.registrationNumber || \'\'}'
);
...
console.log('Patched CorporacionForm.jsx safely!');
